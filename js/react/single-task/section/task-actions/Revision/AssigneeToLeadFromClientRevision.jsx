...
const AssigneeToLeadFromClientRevision = ({
    close,
    onBack,
    onSubmit,
    task,
    ...
    const handleBlurEvent = () => {
        Swal.fire({
            ...
            if (res.isConfirmed) {
                window.open(
                    `/account/projects/${task?.projectId}?tab=milestones`,
                    "_blank"
                );
            }
        });
    ...
    const validate = () => {
        ...
        if (comment === "") {
            errorCount++;
            setCommentError(
                "You have to explain the revision in details, so that lead developer can understand where they need to work."
            );
        }
    ...
    const handleSubmission = (e) => {
        ...
        const data = {
            ...
            dispute_create:
                reason?.id === "CPRx01"
                    ? true
                    : reason?.createDispute ||
                      additionalInfo?.disputeCreate ||
                      false,
        };
    ...
    return (
        <React.Fragment>
            <form className="px-3">
                <div className="form-group">
                    ...
                    <div className="px-3">
                        {_.map(projectManagerAcknowladgement, (option) => {
                            if (!isDesignerTask && option.id === "CPRx07")
                                return null;
                            return (
                                <div
                                    key={option.id}
                ...
                {reason?.id === "CPRx01" && (
                    <div className="form-group">
                        <label htmlFor="" className="d-block font-weight-bold">
                            Is the client paying additionally for these changes?{" "}
                            <sup>*</sup>
                        </label>
                ...
                {additionalPaid === "no" && (
                    <div className="form-group">
                        <label htmlFor="" className="d-block font-weight-bold">
                            Why client is not paying additionally for these
                            changes? <sup>*</sup>
                        </label>
                        <div className="d-block">
                            <div className="form-check mb-3">
                                ...
                                <label
                                    ...
                                    className="form-check-label"
                                >
                                    Client changed his/her mind and he/she don't
                                    want to pay additional payment. We have to
                                    continue the task for client satisfaction
                                </label>
                            </div>
                            <div className="form-check">
                                ...
                                <label
                                    ...
                                    className="form-check-label"
                                >
                                    The client is interpreting his original
                                    instruction in a very unusual way!
                                </label>
                            </div>
                        </div>
                    </div>
